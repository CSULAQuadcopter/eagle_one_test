...
int main(int argc, char** argv)
{
    ...
    /* Message queue length is just 1 for each*/
    pub_twist = node.advertise<geometry_msgs::Twist>("/cmd_vel", queue);
	pub_empty_takeoff = node.advertise<std_msgs::Empty>("/ardrone/takeoff", queue);
	pub_empty_land = node.advertise<std_msgs::Empty>("/ardrone/land", queue);
    pub_empty_reset = node.advertise<std_msgs::Empty>("/ardrone/reset", queue);
